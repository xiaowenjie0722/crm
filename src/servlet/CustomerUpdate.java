...
public class CustomerUpdate extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int num = 0;
        try {
           num=cs.customerUpdate(username,realname,email,phone,address,state,id,limit_id);
        ...
        if (num>0){
            ...
            try {
                String log = TimeUtil.time() + " 用户：" + session.getAttribute("name") + " 编号：" + session.getAttribute("id") + " 操作：修改客户：" + username;
